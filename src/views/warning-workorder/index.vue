<template>
  <div class="warning-workorder-page">
    <!-- 左侧侧边栏 -->
    <div class="sidebar">
      <a-menu
    ...
    <div class="stats-cards">
      ...
      <a-card :bordered="false" class="stat-card total-card">
        <div class="card-header">
          <span class="card-title">预警总数</span>
          <bell-outlined class="card-icon" />
        </div>
        <div class="card-content">
          <div class="card-number">86</div>
          <div class="card-subtitle">今日累计</div>
        </div>
      ...
      <a-card :bordered="false" class="stat-card red-card">
        <div class="card-header">
          <span class="card-title">红色预警 (特急)</span>
          <exclamation-circle-outlined class="card-icon" />
        </div>
        <div class="card-content">
          <div class="card-number red">5</div>
          <div class="card-subtitle red">未处置</div>
        </div>
      ...
      <a-card :bordered="false" class="stat-card orange-card">
        <div class="card-header">
          <span class="card-title">橙色预警 (紧急)</span>
          <warning-outlined class="card-icon" />
        </div>
        <div class="card-content">
          <div class="card-number orange">12</div>
          <div class="card-subtitle orange">未办结</div>
        </div>
      ...
      <a-card :bordered="false" class="stat-card yellow-card">
        <div class="card-header">
          <span class="card-title">黄色预警 (一般紧急)</span>
          <info-circle-outlined class="card-icon" />
        </div>
        <div class="card-content">
          <div class="card-number yellow">28</div>
          <div class="card-subtitle yellow">未办结</div>
        </div>
      ...
      <a-card :bordered="false" class="stat-card blue-card">
        <div class="card-header">
          <span class="card-title">蓝色预警 (提示)</span>
          <bell-outlined class="card-icon" />
        </div>
        <div class="card-content">
          <div class="card-number blue">41</div>
          <div class="card-subtitle blue">未办结</div>
        </div>
      ...
      <a-card :bordered="false" class="stat-card rate-card">
        <div class="card-header">
          <span class="card-title">预警办结率</span>
          <line-chart-outlined class="card-icon" />
        </div>
        <div class="card-content">
          <div class="card-number rate">68.6%</div>
          <div class="card-subtitle rate">逾期: 3条</div>
        </div>
    ...
    <div class="view-switch">
      <a-button-group>
        <a-button type="primary" @click="viewMode = 'list'">
          <table-outlined /> 列表视图
        </a-button>
        <a-button @click="viewMode = 'map'">
        ...
        <a-row :gutter="[16, 16]">
          <a-col :span="6">
            <a-form-item label="时间范围">
              <a-select v-model:value="searchForm.timeRange" style="width: 100%">
          ...
          <a-col :span="6">
            <a-form-item label="预警等级">
              <a-select v-model:value="searchForm.level" style="width: 100%">
          ...
          <a-col :span="6">
            <a-form-item label="预警类型">
              <a-select v-model:value="searchForm.type" style="width: 100%">
                <a-select-option value="all">全部类型</a-select-option>
                <a-select-option value="road">道路积水</a-select-option>
                <a-select-option value="water">管网水位超限</a-select-option>
                <a-select-option value="rain">雨量超标</a-select-option>
                <a-select-option value="equipment">泵站设备故障</a-select-option>
                <a-select-option value="sewage">污水溢流</a-select-option>
                <a-select-option value="manhole">井盖缺失/移位</a-select-option>
                <a-select-option value="quality">水质超标</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="6">
            <a-form-item label="所属片区">
              <a-select v-model:value="searchForm.area" style="width: 100%">
                <a-select-option value="all">全部片区</a-select-option>
                <a-select-option value="main">主城区</a-select-option>
                <a-select-option value="xinan">新安江街道</a-select-option>
                <a-select-option value="yangxi">洋溪街道</a-select-option>
                <a-select-option value="genglou">更楼街道</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="6">
            <a-form-item label="预警状态">
              <a-select v-model:value="searchForm.status" style="width: 100%">
                <a-select-option value="all">全部状态</a-select-option>
                <a-select-option value="pending">未派发</a-select-option>
                <a-select-option value="processing">处置中</a-select-option>
                <a-select-option value="overdue">逾期未处置</a-select-option>
                <a-select-option value="dispatched">已派发</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="12">
    ...
    <div class="action-buttons">
      <a-space>
        <a-button type="primary" style="background-color: #52c41a; border-color: #52c41a;">
          <check-circle-outlined /> 批量接单
        </a-button>
        <a-button type="primary">
          <user-add-outlined /> 批量派单
        </a-button>
        <a-button type="primary" style="background-color: #722ed1; border-color: #722ed1;">
...
const dataSource = ref([
  {
    ...
    level: 'red',
    type: '道路积水',
    address: '建德市主城区 新安江路 与 世纪大道交叉口',
    device: 'JC-2001, CAM-89',
    triggerValue: '45cm',
    threshold: '≤30cm',
    triggerTime: '2024-06-15 08:23:15',
    status: 'processing',
    responsible: '张师傅 13857612345',
    remainingTime: '01:25:33'
  },
  {
    id: 'YJ20240615000002',
    level: 'red',
    type: '管网水位超限',
    address: '新安江街道 环城西路 排水管网段',
    device: 'WT-1028',
    triggerValue: '4.2m',
    threshold: '≤3.5m',
    triggerTime: '2024-06-15 07:45:08',
    status: 'pending',
    responsible: '-',
    remainingTime: '03:12:15'
  },
  {
    id: 'YJ20240615000003',
    level: 'yellow',
    type: '雨量超标',
    address: '洋溪街道 雨水量监测站 3号点',
    device: 'Y-005',
    triggerValue: '55mm/h',
    threshold: '≤40mm/h',
    triggerTime: '2024-06-15 09:10:22',
    status: 'processing',
    responsible: '李师傅 13587654321',
    remainingTime: '04:30:12'
  },
  {
    id: 'YJ20240615000004',
    level: 'red',
    type: '泵站设备故障',
    address: '更楼街道 第一泵站 1号机组',
    device: 'P-001',
    triggerValue: '电流异常 15A',
    threshold: '≤10A',
    triggerTime: '2024-06-15 06:30:45',
    status: 'dispatched',
    responsible: '王师傅 13957612345',
    remainingTime: '12:15:08'
  },
  {
    id: 'YJ20240615000005',
    level: 'red',
    type: '污水溢流',
    address: '建德市主城区 江滨公园 截流井',
    device: 'O-012',
    triggerValue: '1.2m³/s',
    threshold: '≤0.5m³/s',
    triggerTime: '2024-06-15 08:55:33',
    status: 'overdue',
    responsible: '赵师傅 13657612345',
    remainingTime: '-00:35:10'
  },
  {
    id: 'YJ20240615000006',
    level: 'red',
    type: '道路积水',
    address: '建德市主城区 迎宾大道 下穿立交',
    device: 'WL-008, CAM-12',
    triggerValue: '82cm',
    threshold: '≤30cm',
    triggerTime: '2024-06-15 09:05:12',
    status: 'pending',
    responsible: '-',
    remainingTime: '02:45:18'
  },
  {
    id: 'YJ20240615000007',
    level: 'yellow',
    type: '井盖缺失/移位',
    address: '新安江街道 环湖北路 路灯旁',
    device: '人工上报',
    triggerValue: '缺失',
    threshold: '完好',
    triggerTime: '2024-06-15 07:15:40',
    status: 'processing',
    responsible: '刘师傅 13757612345',
    remainingTime: '05:20:40'
  },
  {
    id: 'YJ20240615000008',
    level: 'yellow',
    ...
    address: '梅城镇 出水口断面',
    device: 'WQ-023',
    triggerValue: 'COD 58mg/L',
    threshold: '≤50mg/L',
    triggerTime: '2024-06-15 05:20:18',
    status: 'pending',
    responsible: '-',
    remainingTime: '06:18:22'
  }
])
...
// 初始化环形图
const initRingChart = (element: HTMLElement | undefined, value: number, total: number, color: string) => {
  if (!element) return
  ...
  chart.setOption({
    series: [{
      ...
      data: [
        ...
        { value: total - value, itemStyle: { color: '#e8e8e8' } }
      ],
      label: {
        show: false
      },
